...
namespace VulkanGameEngineLevelEditor.Systems
{
    ...
    public static unsafe class SpriteSystem
    {
        ...
        private static void UpdateSpriteBatchLayers(float deltaTime)
        {
            ...
            {
                SpriteSystem.FindSpriteInstanceList(spriteBatchLayer.SpriteBatchLayerId).Clear();
                var spritelayer = SpriteSystem.FindSpriteInstanceList(spriteBatchLayer.SpriteBatchLayerId);
        ...
        private static void UpdateSprites(float deltaTime)
        {
            for (int x = 0; x < SpriteInstanceList.Count; x++)
            {
                ...
                SpriteList[x] = sprite;
                var transformComponent = GameObjectSystem.Transform2DComponentMap[(int)sprite.GameObjectId];
                transformComponent.GameObjectPosition += new vec2(0.01f, 0f);
                GameObjectSystem.Transform2DComponentMap[(int)sprite.GameObjectId] = transformComponent;
            }
        }
