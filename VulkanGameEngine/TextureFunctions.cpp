...
{
	return Texture_CreateTextureImage(cRenderer.Device, cRenderer.PhysicalDevice, &texture.Image, &texture.Memory, texture.Width, texture.Height, texture.MipMapLevels, texture.TextureByteFormat);
}
...
{
	return Texture_CreateTextureImage(cRenderer.Device, cRenderer.PhysicalDevice, &image, &memory, static_cast<int>(textureSize.x), static_cast<int>(textureSize.y), mipmapLevels, textureByteFormat);
}
...
{
	return Texture_CreateTextureImage(cRenderer.Device, cRenderer.PhysicalDevice, &image, &memory, textureSize.x, textureSize.y, mipmapLevels, textureByteFormat);
}
...
{
	return Texture_CreateTextureView(cRenderer.Device, &texture.View, texture.Image, texture.TextureByteFormat, texture.MipMapLevels);
}
...
{
	return Texture_CreateTextureView(cRenderer.Device, &view, image, format, mipmapLevels);
}

VkResult TextureFunctions::CreateTextureSampler(VkSamplerCreateInfo* samplerCreateInfo, VkSampler* smapler)
{
	return Texture_CreateTextureSampler(cRenderer.Device, samplerCreateInfo, smapler);
}

VkResult TextureFunctions::TransitionImageLayout(Texture texture, VkImageLayout newLayout)
{
	return Texture_QuickTransitionImageLayout(cRenderer.Device, cRenderer.CommandPool, cRenderer.SwapChain.GraphicsQueue, texture.Image, texture.MipMapLevels, &texture.TextureImageLayout, &newLayout);
}
...
{
	return Texture_QuickTransitionImageLayout(cRenderer.Device, cRenderer.CommandPool, cRenderer.SwapChain.GraphicsQueue, texture.Image, texture.MipMapLevels, &oldLayout, &newLayout);
}
...
{
	return Texture_CopyBufferToTexture(cRenderer.Device, cRenderer.CommandPool, cRenderer.SwapChain.GraphicsQueue, texture.Image, buffer, texture.TextureUsage, texture.Width, texture.Height, texture.Depth);
}
...
{
	return Texture_CopyBufferToTexture(cRenderer.Device, cRenderer.CommandPool, cRenderer.SwapChain.GraphicsQueue, image, buffer, textureType, static_cast<int>(textureSize.x), static_cast<int>(textureSize.y), static_cast<int>(textureSize.z));
}
...
{
	return Texture_CopyBufferToTexture(cRenderer.Device, cRenderer.CommandPool, cRenderer.SwapChain.GraphicsQueue, image, buffer, textureType, textureSize.x, textureSize.y, textureSize.z);
}
...
{
	return Texture_CopyBufferToTexture(cRenderer.Device, cRenderer.CommandPool, cRenderer.SwapChain.GraphicsQueue, image, buffer, textureType, width, height, depth);
}

VkResult TextureFunctions::GenerateMipmaps(Texture& texture)
{
	return Texture_GenerateMipmaps(cRenderer.Device, cRenderer.PhysicalDevice, cRenderer.CommandPool, cRenderer.SwapChain.GraphicsQueue, texture.Image, &texture.TextureByteFormat, texture.MipMapLevels, texture.Width, texture.Height);
}
...
{
	return Texture_GenerateMipmaps(cRenderer.Device, cRenderer.PhysicalDevice, cRenderer.CommandPool, cRenderer.SwapChain.GraphicsQueue, image, &textureByteFormat, mipmapLevels, textureSize.x, textureSize.y);
}

VkResult TextureFunctions::GenerateMipmaps(VkImage image, VkFormat& textureByteFormat, uint32 mipmapLevels, vec2 textureSize)
{
	return Texture_GenerateMipmaps(cRenderer.Device, cRenderer.PhysicalDevice, cRenderer.CommandPool, cRenderer.SwapChain.GraphicsQueue, image, &textureByteFormat, mipmapLevels, static_cast<int>(textureSize.x), static_cast<int>(textureSize.y));
}
