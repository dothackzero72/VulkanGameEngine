...
FrameBufferRenderPass::FrameBufferRenderPass(const String& jsonPath, SharedPtr<Texture> inputTexture, ivec2 renderPassResolution)
{
    ...
    JsonPipelineList.emplace_back(std::make_shared<JsonPipeline>(JsonPipeline("../Pipelines/FrameBufferPipeline.json", RenderPass, import, vertexBinding, vertexAttribute, sizeof(SceneDataBuffer))));
}
...
VkCommandBuffer FrameBufferRenderPass::Draw()
{
    ...
    vkCmdBindDescriptorSets(CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, JsonPipelineList[0]->PipelineLayout, 0, 1, &JsonPipelineList[0]->DescriptorSetList, 0, nullptr);
