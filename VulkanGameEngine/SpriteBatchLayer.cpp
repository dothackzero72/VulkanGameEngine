...
SpriteBatchLayer::SpriteBatchLayer(List<SharedPtr<Sprite>> spriteList)
{
	...
	SpriteBuffer = std::make_shared<SpriteInstanceBuffer>(SpriteInstanceBuffer(SpriteInstanceList, SpriteLayerMesh->GetMeshBufferUsageSettings(), SpriteLayerMesh->GetMeshBufferPropertySettings(), true));
...
void SpriteBatchLayer::Update(float deltaTime)
{
	...
	//auto sdf2 = SpriteBuffer->CheckBufferContents();
	//int a = 34;
}
