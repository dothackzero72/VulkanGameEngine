...
VkPipelineShaderStageCreateInfo Pipeline_CreateShader(VkDevice device, const String& filename, VkShaderStageFlagBits shaderStages)
{
...
VkPipeline Pipeline_CreatePipeline(VkDevice device, VkRenderPass renderpass, VkPipelineLayout pipelineLayout, VkPipelineCache pipelineCache, const RenderPipelineModel& model, ivec2& extent)
{
    ...
    {
        Pipeline_CreateShader(device, model.VertexShaderPath, VK_SHADER_STAGE_VERTEX_BIT),
        Pipeline_CreateShader(device, model.FragmentShaderPath, VK_SHADER_STAGE_FRAGMENT_BIT)
    };
