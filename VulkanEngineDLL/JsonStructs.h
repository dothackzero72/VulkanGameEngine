...
public:
    ...
    static RenderedTextureInfoModel from_json(const nlohmann::json& json, ivec2 textureResolution)
    {
        ...
        model.ImageCreateInfo = Json_LoadImageCreateInfo(json["ImageCreateInfo"], textureResolution);
        model.SamplerCreateInfo = Json_LoadVulkanSamplerCreateInfo(json["SamplerCreateInfo"]);
        model.AttachmentDescription = Json_LoadAttachmentDescription(json["AttachmentDescription"]);
        return model;
    }
};
...
public:
    ...
    static RenderPassBuildInfoModel from_json(const nlohmann::json& json, ivec2 textureResolution)
    {
        ...
        {
            model.SubpassDependencyModelList.emplace_back(Json_LoadSubpassDependency(json["SubpassDependencyList"][x]));
        }
        for (int x = 0; x < json["ClearValueList"].size(); x++)
        {
            model.ClearValueList.emplace_back(Json_LoadClearValue(json["ClearValueList"][x]));
        }
        return model;
    }
};
...
struct RenderPipelineModel : RenderPassEditorBaseModel
{
    ...
    static RenderPipelineModel from_json(const nlohmann::json& json)
    {
        ...
        model.PipelineRasterizationStateCreateInfo = Json_LoadPipelineRasterizationStateCreateInfo(json["PipelineRasterizationStateCreateInfo"]);
        model.PipelineMultisampleStateCreateInfo = Json_LoadPipelineMultisampleStateCreateInfo(json["PipelineMultisampleStateCreateInfo"]);
        model.PipelineDepthStencilStateCreateInfo = Json_LoadPipelineDepthStencilStateCreateInfo(json["PipelineDepthStencilStateCreateInfo"]);
        model.PipelineInputAssemblyStateCreateInfo = Json_LoadPipelineInputAssemblyStateCreateInfo(json["PipelineInputAssemblyStateCreateInfo"]);

        for (int x = 0; x < json["PipelineColorBlendAttachmentStateList"].size(); x++)
        {
            model.PipelineColorBlendAttachmentStateList.emplace_back(Json_LoadPipelineColorBlendAttachmentState(json["PipelineColorBlendAttachmentStateList"][x]));
        }
        model.PipelineColorBlendStateCreateInfoModel = Json_LoadPipelineColorBlendStateCreateInfo(json["PipelineColorBlendStateCreateInfoModel"]);

        for (int x = 0; x < json["LayoutBindingList"].size(); x++)
        {
            model.LayoutBindingList.emplace_back(Json_LoadLayoutBinding(json["LayoutBindingList"][x]));
        }
        ...
        {
            model.ViewportList.emplace_back(Json_LoadViewPort(json["ViewportList"][x]));
        }
        for (int x = 0; x < json["ScissorList"].size(); x++)
        {
            model.ScissorList.emplace_back(Json_LoadRect2D(json["ScissorList"][x]));
        }
        for (int x = 0; x < json["VertexInputBindingDescriptionList"].size(); x++)
        {
            model.VertexInputBindingDescriptionList.emplace_back(Json_LoadVertexInputBindingDescription(json["VertexInputBindingDescriptionList"][x]));
        }
        for (int x = 0; x < json["VertexInputAttributeDescriptionList"].size(); x++)
        {
            model.VertexInputAttributeDescriptionList.emplace_back(Json_LoadVertexInputAttributeDescription(json["VertexInputAttributeDescriptionList"][x]));
        }
        return model;
    }
};