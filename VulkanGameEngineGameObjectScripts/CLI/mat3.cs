...
public struct mat3 : IReadOnlyList<float>, IReadOnlyCollection<float>, IEnumerable<float>, IEnumerable, IEquatable<mat3>
{
    //
    // Summary:
    //     Column 0, Rows 0
    ...
    public float[] Values1D => new float[9] { m00, m01, m02, m10, m11, m12, m20, m21, m22 };
    ...
    public mat3 Adjugate => new mat3(m11 * m22 - m21 * m12, (0f - m01) * m22 + m21 * m02, m01 * m12 - m11 * m02, (0f - m10) * m22 + m20 * m12, m00 * m22 - m20 * m02, (0f - m00) * m12 + m10 * m02, m10 * m21 - m20 * m11, (0f - m00) * m21 + m20 * m01, m00 * m11 - m10 * m01);
    ...
    public bool Equals(mat3 rhs)
    {
        if (m00.Equals(rhs.m00) && m01.Equals(rhs.m01) && m02.Equals(rhs.m02) && m10.Equals(rhs.m10) && m11.Equals(rhs.m11))
        {
            if (m12.Equals(rhs.m12) && m20.Equals(rhs.m20))
            {
                if (m21.Equals(rhs.m21))
                {
                    return m22.Equals(rhs.m22);
                }
    ...
    {
        return Math.Pow(Math.Pow(Math.Abs(m00), p) + Math.Pow(Math.Abs(m01), p) + Math.Pow(Math.Abs(m02), p) + (Math.Pow(Math.Abs(m10), p) + Math.Pow(Math.Abs(m11), p)) + (Math.Pow(Math.Abs(m12), p) + Math.Pow(Math.Abs(m20), p) + (Math.Pow(Math.Abs(m21), p) + Math.Pow(Math.Abs(m22), p))), 1.0 / p);
    }
    ...
    {
        return new vec3(m.m00 * v.x + m.m10 * v.y + m.m20 * v.z, m.m01 * v.x + m.m11 * v.y + m.m21 * v.z, m.m02 * v.x + m.m12 * v.y + m.m22 * v.z);
    }
    ...
    public static mat3 CompMul(mat3 A, mat3 B)
    {
        return new mat3(A.m00 * B.m00, A.m01 * B.m01, A.m02 * B.m02, A.m10 * B.m10, A.m11 * B.m11, A.m12 * B.m12, A.m20 * B.m20, A.m21 * B.m21, A.m22 * B.m22);
    }

    //
    // Summary:
    //     Executes a component-wise / (divide).
    public static mat3 CompDiv(mat3 A, mat3 B)
    {
        return new mat3(A.m00 / B.m00, A.m01 / B.m01, A.m02 / B.m02, A.m10 / B.m10, A.m11 / B.m11, A.m12 / B.m12, A.m20 / B.m20, A.m21 / B.m21, A.m22 / B.m22);
    }
    ...
    public static mat3 CompAdd(mat3 A, mat3 B)
    {
        return new mat3(A.m00 + B.m00, A.m01 + B.m01, A.m02 + B.m02, A.m10 + B.m10, A.m11 + B.m11, A.m12 + B.m12, A.m20 + B.m20, A.m21 + B.m21, A.m22 + B.m22);
    }
    ...
    public static mat3 CompSub(mat3 A, mat3 B)
    {
        return new mat3(A.m00 - B.m00, A.m01 - B.m01, A.m02 - B.m02, A.m10 - B.m10, A.m11 - B.m11, A.m12 - B.m12, A.m20 - B.m20, A.m21 - B.m21, A.m22 - B.m22);
    }
    ...
    public static mat3 operator +(mat3 lhs, mat3 rhs)
    {
        return new mat3(lhs.m00 + rhs.m00, lhs.m01 + rhs.m01, lhs.m02 + rhs.m02, lhs.m10 + rhs.m10, lhs.m11 + rhs.m11, lhs.m12 + rhs.m12, lhs.m20 + rhs.m20, lhs.m21 + rhs.m21, lhs.m22 + rhs.m22);
    }
    ...
    {
        return new mat3(lhs.m00 + rhs, lhs.m01 + rhs, lhs.m02 + rhs, lhs.m10 + rhs, lhs.m11 + rhs, lhs.m12 + rhs, lhs.m20 + rhs, lhs.m21 + rhs, lhs.m22 + rhs);
    }
    ...
    {
        return new mat3(lhs + rhs.m00, lhs + rhs.m01, lhs + rhs.m02, lhs + rhs.m10, lhs + rhs.m11, lhs + rhs.m12, lhs + rhs.m20, lhs + rhs.m21, lhs + rhs.m22);
    }
